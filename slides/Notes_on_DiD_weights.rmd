...
author: "By Hans H. Siever tsen (h.h.sievertsen@bristol.ac.uk)"
date: "This version: 06/06/2020; [link to most recent version](https://github.com/hhsievertsen/twowayfedecomp/raw/master/slides/Notes_on_DiD_weights.pdf)"
fontsize: 8pt
output:
...
```{r c2, echo = FALSE,fig.width=1.25,fig.height=.75,fig.align="center",dpi=500}
ggplot(df%>%filter(G!=3))+geom_jitter(aes(x=t,y=y,colour=as.factor(G)),size=.1,alpha=0.1,show.legend = FALSE)+
  geom_point(df%>%filter(G!=3,row_number()==1),mapping=aes(x=t,y=y,colour=as.factor(G)),size=.001,alpha=1)+
  theme_classic()+ theme(legend.position="top")+
  labs(x="t",y="y",colour="Group") +scale_color_manual(values=c("#999999", "#E69F00", "#56B4E9"))+ylim(0,4)+
   theme(axis.ticks = element_blank(),plot.background = element_rect(fill="white",colour = "white"),
         text = element_text(size=3),legend.background = element_rect(fill="white",colour = "white"),
         axis.text=element_text(colour="black"),plot.margin = margin(-.25,0,0,0, "cm"),
         panel.background = element_rect(fill = "white"),legend.margin=margin(b=-10),legend.box.margin=margin(0,0,0,0))+
   geom_vline(xintercept = 10, linetype="dashed", color = "gray", size=0.15)+
  coord_fixed(ratio = 6)
